...
namespace Toll_Collection_System
{
        ...
        static async Task Main(string[] args)
        {
            try
            {
                using (IRepository<Vehicle> vehicles = VehicleRepository.Instance)
                {
                    ...
                    Console.WriteLine($"Vehicle {vehicleToUpdate.Id} successfully updated");
                    Console.WriteLine(vehicleToUpdate.ToString());

                    if (vehicles.Delete(vehicleToUpdate))
                        Console.WriteLine($"Vehicle {vehicleToUpdate.Id} successfully deleted");
                    ...
                    Console.WriteLine();
                    Console.WriteLine($"Total Vehicles found by asynchronously: {searchResultAsync.Count()}");
                    Console.WriteLine("----------------------------------");
    ...
    public interface IRepository<T> : IDisposable, IEnumerable<T> where T : IBaseModel
    {
        ...
        Task<IEnumerable<T>> SearchAsync(string value);
    }
    ...
    public sealed class VehicleRepository : IRepository<Vehicle>
    {
        ...
        private VehicleRepository()
        {
            _data = new List<Vehicle>
            {
                ...
                new Vehicle(3, "Khulna-7890",VehicleType. Car , 100),
